...
$blog=intval($_GET["blog"]); if($blog==0) {$blog=mysqli_fetch_assoc(mysqli_query($db,"select * from blog order by id desc")); $blog=$blog["auto_id"];}
...
if($_POST["add_yeni_sekil"])
{
	...
	foreach($sekil_hardans as $sekil_hardan){
		...
		if($sekil_olar==true)
		{
			...
			mysqli_query($db,"insert into $do values (0,'$time','$tip','$blog') ");

			$last_id=mysqli_insert_id($db);
			move_uploaded_file($sekil_hardan,'../images/news_gallery/'.$last_id.'.'.$tip);
			chmod('../images/news_gallery/'.$last_id.'.'.$tip, 0755);
			//Thumb yaradir
			$uploaded_sekil='../images/news_gallery/'.$last_id.'.'.$tip;
...
elseif($delete>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where id='$delete' "))>0)
{
	...
	@unlink('../images/news_gallery/'.$delete.'.'.$tip);
...
<div class="onecolumn">
	...
	<div class="content">
		...
		?>
		<!-- Content start-->
		<form action="index.php?do=<?php echo $do; ?><?php if($blog>0) echo '&blog='.$blog; if($edit>0) echo '&edit='.$edit; if($add==1) echo '&add='.$add;?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
			<hr class="clear" />

			Xəbərlər:&nbsp;&nbsp;&nbsp;
			<select name="sorgu" onchange="MM_jumpMenu('parent',this,0)">
				<?php
				$sql=mysqli_query($db,"select * from blog where lang_id='$main_lang' order by auto_id desc");
				while($row=mysqli_fetch_assoc($sql))
				{
					if($row["auto_id"]==$blog) echo '<option value="index.php?do='.$do.'&blog='.$row["auto_id"].'" selected="selected">'.$row["name"].'</option>';
					else echo '<option value="index.php?do='.$do.'&blog='.$row["auto_id"].'">'.$row["name"].'</option>';
				}
				?>
			</select>
			<br class="clear" /><br />
			...
			<ul class="media_photos">
				<?php
				$sql=mysqli_query($db,"select * from $do where blog_id='$blog' order by id");
				while($row=mysqli_fetch_assoc($sql))
				{
					echo '<li style="margin-bottom:20px">
				  <a rel="slide" href="../images/news_gallery/'.$row["id"].'.'.$row["tip"].'" title="">
				  	<img src="../images/news_gallery/'.$row["id"].'.'.$row["tip"].'" alt="" width="75" height="75" />
				  </a>
				  <br class="clear" />
				  '.$row["basliq"].' <a href="index.php?do='.$do.'&delete='.$row["id"].'&blog='.$blog.'" title="Sil" class="delete"><img src="images/icon_delete.png" alt="" /></a>
			</li>';
				}
				?>
			</ul>
