...
if($_POST) // Add && edit
{
    ...
    {
        $question="question_".$row["id"]; $question=mysqli_real_escape_string($db,htmlspecialchars($$question));
        $answer="answer_".$row["id"]; $answer=mysqli_real_escape_string($db,htmlspecialchars($$answer));
...
//        if($name=='' && $row["id"]!=$main_lang) {$c='name'; $$c=get_lang_val($do,$auto_id,$c);}
        ...
        {
            mysqli_query($db,"update $do set question='$question',answer='$answer',updated_at='$time' where lang_id='$row[id]' $add_where");
        }
        else
        {
            mysqli_query($db,"insert into $do values (0,'$question','$answer','$last_order', '$active', '$row[id]', '$auto_id', '$time',0) ");
        }
    }
