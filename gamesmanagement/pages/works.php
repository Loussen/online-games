...
if($_POST) // Add && edit
{
    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        {
            mysqli_query($db,"update $do set name='$name',title='$title',link='$link',qisa_metn='$qisa_metn',text='$text',parent_auto_id='$parent_auto_id', sira='$son_sira', manset='$manset', updated_at='".time()."' where lang_id='$row[id]' $add_where");
        ...
        else mysqli_query($db,"insert into $do values (0,'$name','$title','$link','$qisa_metn','','$text', '$manset','$parent_auto_id','$son_sira', '$aktivlik', '$row[id]', '$auto_id', '0', '".time()."','0') ");
        ...
        if($sekil_hardan!="")
        {
            ...
            if($sekil_olar==true)
            {
                ...
                move_uploaded_file($sekil_hardan,'../images/works/'.$this_id.'.'.$tip);
                mysqli_query($db,"update $do set tip='$tip' where auto_id='$this_id'");
                //Thumb yaradir
//                $uploaded_sekil='../images/works/'.$this_id.'.'.$tip;
//                checkMaxSizeImage($uploaded_sekil,$maxWidth=460,$maxHeight=300);
//                $thumbFile='../images/works/'.$this_id.'_thumb.'.$tip;
//                makeThumb($uploaded_sekil,$thumbFile,130,85);
                //
            }
        }
...
elseif($delete_img>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete_img' and tip!='' "))>0)
{
    ...
    $ok="Şəkil silindi.";
    @unlink('../images/works/'.$delete_img.'.'.$info_tip["tip"]);
}
...
<div class="onecolumn">
    <div class="header">
        <span>İşlərimiz <?php if($edit>0) echo '(ID: '.$edit.')'; ?></span>
        <div class="switch">
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?><?php if($edit>0) echo '&edit='.$edit.''; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            while($row=mysqli_fetch_assoc($sql))
            {
                ...
                echo '  Başlıq:<br />
			  <input type="text" name="name_'.$row["id"].'" value="'.$information["name"].'" style="width:250px" /><br />
			  <br />
			  <br />
			  Qısa mətn (120 simvol):<br />
			<textarea name="qisa_metn_'.$row["id"].'" rows="10" style="width:55%;">'.$information["qisa_metn"].'</textarea>
			<br /><br />
			Text:<br />
            ...
            echo '<div class="'.$hide.'">';
            ?>
            Manset : <input type="checkbox" name="manset" <?=$information['manset']==1 ? 'checked' : ''?> /><br /><br />
            <?php
            if($information["tip"]!="" && $edit>0) $sekili='<ul class="media_photos" style="margin-left:450px;margin-top:-25px"><li><a rel="slide" href="../images/works/'.$information["auto_id"].'.'.$information["tip"].'?rand='.rand(0,10000).'" title="">Hazırkı şəkil <img src="../images/works/'.$information ["auto_id"].'.'.$information["tip"].'?rand='.rand(0,10000).'" alt="" width="120" /></a><br class="clear" />
            ...
            if($information["tip"]!="" && $edit>0) echo '<br /><br /><br /><br /><br /><br /><br /><br />';
	...
	<th style="width:40%"><input type="checkbox" data-val="0" name="all_check" id="hamisini_sec" value="all_check" /> Başlıq</th>
	...
	<th style="width:30%">Düzəlişlər</th>
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            ...
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-40px"><li><a rel="slide" href="../images/works/'.$row["auto_id"].'.'.$row["tip"].'?rand='.rand(0,10000).'" title=""><img src="../images/works/'.$row["auto_id"].'.'.$row["tip"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
      ...
      //       echo '
            ...
            if($row["aktivlik"]==1) $title='Active'; else $title='Deactive';
            echo '<img src="images/'.$row["aktivlik"].'_lamp.png" title="'.$title.'" border="0" align="absmiddle" style="cursor:pointer" id="info_'.$row["auto_id"].'" onclick="aktivlik(\''.$do.'\',this.id,this.title)"  />
            <a href="index.php?do=works_gallery&works='.$row['auto_id'].'"><img src="images/fotoqalereya.png" alt="" title="Qalereya" /></a>';
            echo '</td>
				</tr>';
