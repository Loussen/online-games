    ...
    while($row=mysqli_fetch_assoc($sql))
    {
        ...
        if($sekil_hardan!="")
        {
            ...
            if($sekil_olar==true)
            {
                ...
                move_uploaded_file($sekil_hardan,'../images/blog/'.$this_id.'.'.$tip);
                mysqli_query($db,"update $do set tip='$tip' where auto_id='$this_id'");
                //Thumb yaradir
//                $uploaded_sekil='../images/blog/'.$this_id.'.'.$tip;
//                checkMaxSizeImage($uploaded_sekil,$maxWidth=460,$maxHeight=300);
//                $thumbFile='../images/blog/'.$this_id.'_thumb.'.$tip;
...
elseif($delete_img>0 && mysqli_num_rows(mysqli_query($db,"select id from $do where auto_id='$delete_img' and tip!='' "))>0)
{
    ...
    $ok="Şəkil silindi.";
    @unlink('../images/blog/'.$delete_img.'.'.$info_tip["tip"]);
}
...
<div class="onecolumn">
    <div class="header">
        <span>Xəbərlər <?php if($edit>0) echo '(ID: '.$edit.')'; ?></span>
        <div class="switch">
    ...
    <div class="content">
        ...
        <form action="index.php?do=<?php echo $do; ?><?php if($edit>0) echo '&edit='.$edit.''; ?>" method="post" id="form_login" name="form_login" enctype="multipart/form-data">
            ...
            <?php
            if($information["tip"]!="" && $edit>0) $sekili='<ul class="media_photos" style="margin-left:450px;margin-top:-25px"><li><a rel="slide" href="../images/blog/'.$information["auto_id"].'.'.$information["tip"].'?rand='.rand(0,10000).'" title="">Hazırkı şəkil <img src="../images/blog/'.$information ["auto_id"].'.'.$information["tip"].'?rand='.rand(0,10000).'" alt="" width="120" /></a><br class="clear" />
	...
	<th style="width:30%">Düzəlişlər</th>
        ...
        while($row=mysqli_fetch_assoc($sql))
        {
            ...
            echo '<tr>
					...
					<td><ul class="media_photos" style="padding:0;margin-top:0px;margin-bottom:-40px"><li><a rel="slide" href="../images/blog/'.$row["auto_id"].'.'.$row["tip"].'?rand='.rand(0,10000).'" title=""><img src="../images/blog/'.$row["auto_id"].'.'.$row["tip"].'?rand='.rand(0,10000).'" width="100" height="60"></a></li></ul></td>
					<td>
      ...
      //       echo '
            ...
            if($row["aktivlik"]==1) $title='Active'; else $title='Deactive';
            echo '<img src="images/'.$row["aktivlik"].'_lamp.png" title="'.$title.'" border="0" align="absmiddle" style="cursor:pointer" id="info_'.$row["auto_id"].'" onclick="aktivlik(\''.$do.'\',this.id,this.title)"  />
            <a href="index.php?do=news_gallery&blog='.$row['auto_id'].'"><img src="images/fotoqalereya.png" alt="" title="Qalereya" /></a>';
            echo '</td>
				</tr>';
